...
def run_query_method(job):
    config = job
    arch = config['arch']
    accuracy = query(arch, config['performance_metric'], config['dataset'], dataset_api=config['dataset_api'])
    return accuracy
...
#optimizer = RE_predictor(config, transfer_method=my_transfer_method) 
...
with Evaluator.create(
    ...
    if evaluator is not None:
        trainer = Trainer( config, lightweight_output=True)
        trainer.adapt_search_space(search_space, dataset_api=dataset_api)
        if config.optimizer in ["bananas", "npenas", "re"]:
