        ...
        for node_idx in lexicographical_topological_sort(self):
            ...
            if node_idx in pred_graph:
                for pred_idx in pred_graph[node_idx]:
                    ...
                    if pred_name in layer_hash_map:
                        l = layer_hash_map[pred_name]
                        if isinstance(l, str):
                            layer_hash_map[pred_name] = hashlib.sha3_512(layer_hash_map[pred_name].encode()).hexdigest()
                        else:
                            layer_hash_map[pred_name] = layer_hash_map[pred_name].hexdigest()
            ...
            for neigbor_idx in self.neighbors(node_idx):
                ...
                if isinstance(l, str):
                    hashlib.sha3_512(layer_hash_map[config_numbered].encode()).update(raw_config.encode())
                else:
                    layer_hash_map[config_numbered].update(raw_config.encode())
                ...
                if node_idx in pred_graph:
                    for pred_idx in pred_graph[node_idx]:
                        ...
                        l = layer_hash_map[config_numbered]
                        if isinstance(l, str):
                            hashlib.sha3_512(layer_hash_map[config_numbered].encode()).update( hashed_names[pred_name].encode())
                        else:
